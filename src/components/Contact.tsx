...
export default function Contact() {
  return (
    <section id="contact" className="mx-auto max-w-screen-2xl px-6 py-16 sm:px-10 sm:py-24 lg:px-16 xl:px-24">
      <Reveal className="flex flex-col items-center gap-6 rounded-card border border-line bg-surface/90 p-10 text-center shadow-card backdrop-blur-sm sm:p-14">
        ...
        {profile.email ? (
          <a
            ...
            className="inline-flex items-center gap-2 rounded-full bg-coral px-6 py-3 text-sm font-semibold text-foam shadow-card transition-colors hover:bg-coral-2"
          >
            <Mail size={16} />
            {profile.email}
          </a>
          ...
          {profile.socials.map((social) =>
            social.href ? (
              <a
                ...
                className="flex items-center gap-2 rounded-full border border-line-2 bg-surface-2 px-4 py-2 text-sm font-medium text-ink-2 transition-colors hover:border-coral hover:text-coral"
              >
                <LinkIcon size={15} />
                {social.label}
              </a>
